...
type Props = { risks: RiskItem[]; selectedId?: string; filters: RiskFilters; search: string; loading?: boolean; onFilters: (next: RiskFilters) => void; onSearch: (value: string) => void; onSelect: (id: string) => void };
export function RiskInbox({ risks, selectedId, filters, search, loading = false, onFilters, onSearch, onSelect }: Props) {
  const searchRef = useRef<HTMLInputElement>(null);
  useEffect(() => { const focusSearch = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") { event.preventDefault(); searchRef.current?.focus(); } }; globalThis.addEventListener("keydown", focusSearch); return () => globalThis.removeEventListener("keydown", focusSearch); }, []);
  ...
  return <section className="risk-inbox" aria-labelledby="risk-inbox-title">
    <header className="workspace-heading"><div><h1 id="risk-inbox-title">Risk inbox</h1><p>Prioritized coordination risks from the active evidence snapshot.</p></div>
      <label className="risk-search"><Icon name="search"/><span className="sr-only">Search risks</span><input ref={searchRef} placeholder="Search risks, services, teams…" value={search} onChange={(event) => onSearch(event.target.value)}/><kbd>Ctrl K</kbd></label>
    </header>
    ...
    <div className="risk-count"><strong>{loading ? "Loading analyses…" : `${visible.length} risks`}</strong><span>Sorted by priority and confidence</span></div>
    <div className="risk-table-wrap"><table className="risk-table"><thead><tr><th>Priority</th><th>Risk</th><th>Risk type</th><th>Affected area</th><th>Team</th><th>Confidence</th><th>Last observed</th></tr></thead><tbody>
      {visible.map((risk) => <tr aria-selected={risk.id === selectedId} key={risk.id} onClick={() => onSelect(risk.id)}>
        <td data-label="Priority"><button className={`priority priority-${risk.priority.toLowerCase()}`} onClick={() => onSelect(risk.id)} type="button">{risk.priority}</button></td>
        <td data-label="Risk"><button className="risk-title-button" onClick={() => onSelect(risk.id)} type="button">{risk.title}</button></td>
        <td data-label="Type"><span className={`risk-type risk-type-${risk.kind}`}><i/>{riskKindLabel(risk.kind)}</span></td><td data-label="Area">{risk.affectedArea}</td><td data-label="Team">{risk.team}</td>
        <td data-label="Confidence"><strong>{risk.confidence >= 75 ? "High" : risk.confidence >= 50 ? "Medium" : "Low"}</strong><small>{risk.confidence}%</small></td><td data-label="Observed">{risk.lastObserved}</td>
      </tr>)}</tbody></table>
      {loading && visible.length === 0 ? <div className="risk-loading" role="status"><i/><i/><i/><span>Running the evidence lenses…</span></div> : null}
      {!loading && visible.length === 0 ? <div className="risk-empty"><Icon name="filter"/><h2>No risks match these filters</h2><p>Clear a filter or search another service.</p></div> : null}
    </div>
  </section>;
}
